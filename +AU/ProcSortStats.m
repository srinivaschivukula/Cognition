classdef ProcSortStats < handle
    ...
    methods
        ...
        function [Property,unitID,invalidUnits]=returnSortStats(this,PropertyName,unitID)
            Property = Analyze.returnFieldValues(this.SortData,PropertyName);
            invalidUnits=[];
            % subselect output based on defined units if specified.
            if nargin>2
                %%
                AllUnits = Analyze.returnFieldValues(this.SortData,'unit');
                AllDateID = Analyze.returnFieldValues(this.SortData,'DateID');
                
                for i=1:size(unitID,1)
                    inds=find(ismember([AllUnits AllDateID],unitID(i,:),'rows'));
                ...
                if any(isnan(validIDX))
                    invalidUnits=unitID(isnan(validIDX),:);
                    warning('The following could not be found')
                    invalidUnits
                    
                    unitID=unitID(~isnan(validIDX),:);
                    validIDX(isnan(validIDX))=[];
                end
                Property=Property(validIDX,:);
            end
        end
        
        function ValidIDX=plotSortStat(this,PropertyName,unitID)
            
            Property=returnSortStats(this,PropertyName,unitID);
            %%
            figure;
            histogram(Property,floor(length(Property)/5));
            plt.vline(prctile(Property,[25 50 75]),'k')
        end
        
        function ValidIDX=returnValidInds(this,PropertyName,rule,criteria,unitID,varargin)
            
            [varargin,ValueType]   = Utilities.ProcVarargin(varargin,'ValueType','raw');
            Property=returnSortStats(this,PropertyName,unitID);
            
            if strcmp(ValueType,'Percentile')
                criteria=prctile(Property,criteria);
            end
            %
            if strcmp('>=',rule)
                ValidIDX=Property>=criteria;
            elseif strcmp('>',rule)
                ValidIDX=Property>criteria;
