...
%%
GlobalOpts.AnalysesName='FormatCorrDynamic'; % larger window
GlobalOpts.PlotShuffle=0;
Dates=GlobalOpts.Dates;
%%
if GlobalOpts.RunInline
    for i=1:length(Dates)
        AU.PopulationAnalysisThroughTime(@Analyze.FaceScratch3.Config_FormatCorrDynamic ,GlobalOpts,'Dates',Dates(i),'overwrite',1);
    end
end

% if GlobalOpts.RunBatch
%     for i=1:length(Dates)
%         jobInfoDecodeTest{i}=batch(@AU.PopulationAnalysisThroughTime,0,{@Analyze.Semantic.Config_FormatCorrDynamic,GlobalOpts,...
%             'Dates',Dates(i)});
%     end
% end
...
if GlobalOpts.RunPlots
    AU.PlotPopulationAnalThroughTime(@Analyze.FaceScratch3.Config_FormatCorrDynamic ,GlobalOpts,'PopulationOverwrite',1);
end
