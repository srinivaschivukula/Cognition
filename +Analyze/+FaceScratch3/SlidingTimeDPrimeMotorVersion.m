...
function SlidingTimeDPrimeMotorVersion(config,varargin)
...
%     %     BaselineDataFI_null = Analyze.SubSelectTrials(trialdata, 'Phase', 'ITI','Condition',{'XX'});
    ...
    for timeIdx = 1:length(TimeWindows)
        ...
        if cTW(1)>0
            ...
            baseFR = [squeeze(Analyze.getNeuralData(BaselineData_attHand, cUnits, [0 0.5]));squeeze(Analyze.getNeuralData(BaselineData_attShoulder, cUnits, [0 0.5]));...
                squeeze(Analyze.getNeuralData(BaselineData_imagHand, cUnits, [0 0.5]));squeeze(Analyze.getNeuralData(BaselineData_imagShoulder, cUnits, [0 0.5]))];
